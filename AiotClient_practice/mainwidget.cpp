    ...
    , ui(new Ui::MainWidget)
{
    ...
    // 1) dial changed in pTab1 -> activate slot finction in pTab2
    connect(pTab1DeviceControl, SIGNAL(on_pDialLed_valueChanged_sig(QString)),pTab2SocketClient,SLOT(socketWriteDataSlot(QString)));
    connect(pTab2SocketClient, SIGNAL(tab1SendKeyData(QStringList&)),pTab1DeviceControl,SLOT(activateCheckBoxSlot(QStringList&)));
}
